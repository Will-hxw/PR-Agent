...
- 目前 task-backed 事件包括：`CI_FAILURE`、`REVIEW_CHANGES_REQUESTED`、`MAINTAINER_COMMENT`、`BOT_COMMENT`、`NEW_COMMENT`、`NEEDS_REBASE`、`READY_TO_MERGE`。
...
评论 task 成功时，只推进对应 category 的 cursor；其他 category 的 baseline 不会被一并推进。
